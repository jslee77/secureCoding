...
async function loadDocs() {
  ...
  byId("stats").innerHTML = [
    ["전체 문서", docs.length, "🗂️", "ic-orange"],
    ["내 문서", mine, "📝", "ic-blue"],
    ["공개 문서", pub, "🌐", "ic-green"],
    ["팀원", "4+", "👥", "ic-purple"],
  ].map(s => `<div class="stat"><div class="k"><span class="ic ${s[3]}">${s[2]}</span>${s[0]}</div><div class="v">${s[1]}</div></div>`).join("");

  if (!docs.length) { byId("doc-list").innerHTML = '<div class="empty"><div class="big">🗂️</div>아직 문서가 없어요.</div>'; return; }
...
async function loadComments(docId) {
  ...
  if (!r.data.length) { box.innerHTML = '<div class="comment-empty">첫 댓글을 남겨보세요.</div>'; return; }
