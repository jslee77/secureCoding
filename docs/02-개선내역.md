...
8. [운영 완결성 재점검](#8-운영-완결성-재점검-2026-09-22)
...
**개선** — 문서 조회·렌더링·댓글·첨부 다운로드·내보내기 등 **문서를 다루는 모든 경로**에서 `can_access`(소유자 / 공개 / 공유받음)를 확인합니다. 조회·렌더·다운로드·내보내기는 "없음"과 "권한 없음"을 같은 404로 응답합니다. 댓글·편집·공유 등은 경로에 따라 403을 반환하므로 모든 경로의 응답 코드가 동일한 것은 아닙니다.
...
- 중복 여부와 관계없이 **먼저 해시**해 비밀번호 해시 수행 여부에 따른 큰 시간 차이를 줄임(완전한 시간 균일성 보장은 아님)
